        ...
        switch (currentByte) {
            ...
            case TRANSOUT:
                ...
                if (bufAlgo == 0)
                    ...
                    procReturn = proc_TRANS(&(bufAlgo[bufAlgoIndex]), bufAlgoSize, &bufAlgoIndex, getCurrentChannel(),
                                            currentByte);
                if (procReturn <= 0) {
#ifdef    DEBUG_LEVEL_1
        ...
        switch (currentByte) {
            ...
            case TRANSOUT:
                ...
                if (currentByte == ALGODATA) {
...
#ifdef DEBUG_LEVEL_1
                            dbgu_putint(DBGU_L1_ALGO_TRANX,
                                        NO_TRANSOUT_DATA);//"Algo error: no byte available for transmit",
#endif
                            return ERROR_PROC_ALGO;
                        }
                    }
                    ...
                    retVal = TRANS_transceive_stream(0, trBuffer, trCount, DATA_TX, &currentByte, flag_mask,
                                                     maskBuffer);
                    if (retVal <= 0) {
#ifdef DEBUG_LEVEL_1
...
#ifdef DEBUG_LEVEL_1
                            dbgu_putint(DBGU_L1_ALGO_TRANX,
                                        NO_TRANSIN_DATA);//"Algo error: no byte available for transmit",
#endif
                            return ERROR_PROC_ALGO;
                        }
            ...
            case PROGDATAEH:
                if (!flag_transin) {
                    ...
                    retVal = TRANS_transceive_stream(0, trBuffer, trCount, DATA_RX, &currentByte, flag_mask,
                                                     maskBuffer);
                    if (retVal <= 0 && retVal != ERROR_VERIFICATION) {
#ifdef DEBUG_LEVEL_1
                        dbgu_putint(DBGU_L1_TRANX_PROC, TRANX_IN_PROG_FAIL);//"Transmit error: unable to transmit",
